describe('findReplace', function() {
    it('finds a word within a given passage', function() {
        expect(findReplace('hello', 'hello my name is ike')).to.equal('hello')
    });

    it('finds multiple words within a passage', function() {
        expect(findReplace('cat', 'I live with one cat.  The cat belongs to my roommate. The cat is brown and has long cat hair.')).to.equal(['cat', 'cat', 'cat', 'cat'])
    });

    it('finds word regardless of captialization', function() {
        expect(findReplace('carbuncle', 'My roommate\'s cAt is named Carbuncle. carbuncle really likes to chase lasors.')).to.equal(['Carbuncle', 'carbuncle'])
    });
